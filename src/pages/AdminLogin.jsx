...
const AdminLogin = () => {
    ...
    const [forgotPasswordStatus, setForgotPasswordStatus] = useState({ loading: false, success: false, message: '' });
    ...
    const onForgotPasswordSubmit = async (e) => {
        ...
        setForgotPasswordStatus({ loading: true, success: false, message: '' });
        try {
            const response = await api.post('/auth/forgot-password', { email: forgotPasswordEmail });
            if (response.data.success || response.status === 200) {
                setForgotPasswordStatus({ loading: false, success: true, message: 'Password reset link sent to your email.' });
            } else {
                setForgotPasswordStatus({ loading: false, success: false, message: response.data.message || 'Something went wrong.' });
            }
        } catch (error) {
            setForgotPasswordStatus({ loading: false, success: false, message: error.response?.data?.message || 'Failed to send reset link.' });
        }
    };
                ...
                {isForgotPassword ? (
                    <div className="space-y-6">
                        ...
                        {forgotPasswordStatus.message && (
                            <div className={`p-3 rounded-lg text-sm border ${forgotPasswordStatus.success ? 'bg-green-50 text-green-700 border-green-100' : 'bg-red-50 text-red-600 border-red-100'}`}>
                                {forgotPasswordStatus.message}
                            </div>
                        )}
                        ...
                        <form onSubmit={onForgotPasswordSubmit} className="space-y-4">
                            ...
                            <button
                                type="button"
                                onClick={() => { setIsForgotPassword(false); setForgotPasswordStatus({ loading: false, success: false, message: '' }); }}
                        ...
                        <div>
                            <div className="flex justify-between items-center mb-1">
                                ...
                                <button
                                    ...
                                    className="text-sm font-medium text-[#D4AF37] hover:text-[#b8962e]"
                                >
                                    Forgot Password?
                                </button>
                            </div>
